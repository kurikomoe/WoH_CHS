...
parser.add_argument("--output_png", help="output folder to store new font.png", default="font_rebuild_png")
...
with open("mapping_orig_2_chengkong.json", "r", encoding='utf-8') as f:
    mapping_orig_2_chengkong = json.load(f)

with open("mapping_chengkong_2_orig.json", "r", encoding='utf-8') as f:
    mapping_chengkong_2_orig = json.load(f)

with open("orig_ccit.json", 'r', encoding='utf-8') as f:
...
FONT_SIZE = 60
font_regular = ImageFont.truetype(args.font_regular, size=FONT_SIZE)
...
def draw_char(draw, font, pos, text, outline=False):
    ...
    if text in r"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()_+=-[]\;',./<>?:\"{}|":
        font = ImageFont.truetype(args.font_regular, size=FONT_SIZE*0.8)
    ...
    position = (x * M + M//2, (y + 1) * M - 15)
    # position = (x * M + (M - w)//2, y * M - (M-h)//2 + dy)
    # draw.rectangle(draw.textbbox(position, text, font=font, anchor="ms"), outline='red')
    # draw.rectangle(draw.textbbox(position, text, font=font), outline='red')
    if not outline:
        draw.text(position, text, font=font, anchor="ms", align="left", fill=(255, 255, 255, 255))
        # draw.text((x * M + (M - w)//2, y * M - (M-h)//2 + dy), text, font=font, align="mb", fill=(255, 255, 255, 255))
    else:
        draw.text(position, text, font=font, anchor="ms", align="left", fill=(255, 255, 255, 255), stroke_width=3, stroke_fill=(255, 255, 255, 255))
        # draw.text((x * M + (M - w)//2, y * M - (M-h)//2 + dy), text, font=font, align="mb", fill=(255, 255, 255, 255), stroke_width=3, stroke_fill=(255, 255, 255, 255))
    x += 1
    if x == 14:
        x = 0
        y += 1
...
def worker(chars_chunk, suffix):
    ...
    for i in range(29):
        y = i * 1024 // 28
        draw.line([(0, y), (1024, y)], fill="gray", width=2)

    # 画竖线（x方向）
    for i in range(29):
        x = i * 1024 // 28
        draw.line([(x, 0), (x, 1024)], fill="gray", width=2)
    """
    ...
    # img.show()
    # exit(-1)
    output_file = os.path.join(args.output_png, f"font_replace-{suffix}.png")
